...
def workflow_selector():
    url = st.text_input("Workflow repository URL (e.g. https://github.com/snakemake-workflows/rna-seq-kallisto-sleuth)")
...
def workflow_editor(workflow: Workflow) -> tempfile.TemporaryDirectory:
    ...
    deploy(workflow.url, None, tag=workflow.tag, branch=workflow.branch, dest_path=tmpdir)

    # handle config
    conf_path = Path(tmpdir.name) / "config" / "config.yaml"
    if conf_path.exists():
